...
mouse_x == clamp(mouse_x, bbox_left, bbox_right) && mouse_y == clamp(mouse_y, bbox_top, bbox_bottom)) || gamepad_button_check_pressed(0, gp_shoulderl)) {
	with (obj_skin_selecter) {
		if (instance_exists(obj_skins_loader)) {
			...
			if (index == 0) {
				obj_skins_loader.current = ds_list_find_value(obj_skins_loader.player_skins, length - 1);
			} else {
